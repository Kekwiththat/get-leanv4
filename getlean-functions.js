...
function simulate(){
  ...
  new Chart(ctx,{type:'line',data:{labels:Array.from({length:pd},(_,i)=>`Day ${i+1}`),datasets:[{label:'Projected Weight',data:simWeight,borderColor:'purple',fill:false},{label:'Target',data:Array(pd).fill(gw),borderColor:'black',borderDash:[5,5],fill:false}]},options:{responsive:true}});
...
// ======================= CHARTS =======================
let caloriesChart, waterChart, stepsChart, weightChart, mealChart, waterDayChart;
function updateCharts(){
  ...
  if(!caloriesChart){caloriesChart=new Chart(calCtx,{type:'line',data:{labels,datasets:[{label:'Calories',data:caloriesData,borderColor:'green',fill:false},{label:'Goal',data:Array(programDays).fill(calorieGoal),borderColor:'black',borderDash:[5,5],fill:false}]}});}
  else{caloriesChart.data.labels=labels; caloriesChart.data.datasets[0].data=caloriesData; caloriesChart.data.datasets[1].data=Array(programDays).fill(calorieGoal); caloriesChart.update();}
  // Water Chart
  const waterCtx=document.getElementById('waterChart').getContext('2d');
  if(!waterChart){waterChart=new Chart(waterCtx,{type:'line',data:{labels,datasets:[{label:'Glasses',data:waterData,borderColor:'blue',fill:false},{label:'Goal',data:Array(programDays).fill(16),borderColor:'black',borderDash:[5,5],fill:false}]}});}
  else{waterChart.data.labels=labels; waterChart.data.datasets[0].data=waterData; waterChart.update();}
  // Steps Chart
  const stepCtx=document.getElementById('stepsChart').getContext('2d');
  if(!stepsChart){stepsChart=new Chart(stepCtx,{type:'line',data:{labels,datasets:[{label:'Steps',data:stepsData,borderColor:'orange',fill:false},{label:'Goal',data:Array(programDays).fill(stepGoal),borderColor:'black',borderDash:[5,5],fill:false}]}});}
  else{stepsChart.data.labels=labels; stepsChart.data.datasets[0].data=stepsData; stepsChart.update();}
  // Weight Chart
  const weightCtx=document.getElementById('weightChart').getContext('2d');
  if(!weightChart){weightChart=new Chart(weightCtx,{type:'line',data:{labels,datasets:[{label:'Weight',data:weightData,borderColor:'purple',fill:false}]}});}
  else{weightChart.data.labels=labels; weightChart.data.datasets[0].data=weightData; weightChart.update();}
}

// ======================= NOTIFICATIONS =======================
function enableNotifications(){
  if("Notification" in window){Notification.requestPermission().then(p=>{if(p==='granted')alert('Notifications Enabled');});}
}
